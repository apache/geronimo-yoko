...
import testify.jupiter.annotation.impl.SimpleArgumentsProvider;
...
//            nameCount.computeIfAbsent(config.value(), s -> new AtomicInteger()).incrementAndGet();
//        }
...
//                    return new ServerSteward(testClass, cfg);
//                })
...
class MultiServerExtension implements BeforeAllCallback, SimpleArgumentsProvider<Bus> {
    ...
    @Override
    public Stream<Bus> provideArgs(ExtensionContext ctx) { return null;/* return MultiServerSteward.getInstance(ctx).buses(ctx); */ }
}
